...
Bool_t AddFile2Chain(TString Inputfilelist, TChain &tpcChain, TChain &odChain){
  ...
  TString tpcBase, odBase;
  TString tpcFName, odFName;
  TString suffix = ".root";
  ...
  while(inputStream >> word)
    {
      ...
      if(word[0] == '#')
	{
	  inputStream.getline(word, 512);
	  //	  word = "";
	  continue;
	}
      // Read TPC and OD name bases (path + first part of file names)
      if(tpcBase == "" && odBase == "")
	{
	  tpcBase = word;
	  inputStream >> word;
      ...
      // Read TPC run number and OD run number
      tpcFName = tpcBase;
      tpcFName += word;
      tpcFName += "/Run"; tpcFName += word;
      tpcFName += suffix;
      inputStream >> word;
      odFName = odBase;
      odFName += word;
      ...
      // starting with TPC...
      TFile *tpcFTmp = new TFile(tpcFName);
      if(!(tpcFTmp->IsOpen())){
	cout << tpcFName << " open failed ! not chained" << endl;
	continue;
      }
      if(tpcFTmp->IsZombie()){
	cout << "something is very wrong with " << tpcFName << ", not chained" << endl;
	continue;
      }
      if(tpcFTmp->TestBit(1024)){
	cout << "recover procedure applied to " << tpcFName << endl;
	continue;
      }

      // ...now check for the OD
      TFile *odFTmp = new TFile(odFName);
      if(!(odFTmp->IsOpen())){
	cout << odFName << " open failed ! not chained" << endl;
	continue;
      }
      if(odFTmp->IsZombie()){
	cout << "something is very wrong with " << odFName << ", not chained" << endl;
	continue;
      }
      if(odFTmp->TestBit(1024)){
	cout << "recover procedure applied to " << odFName << endl;
	continue;
      }

      // Add the files to their respective chains
      if(tpcFTmp && tpcFTmp->IsOpen() && tpcFTmp->GetNkeys()){
	if(odFTmp && odFTmp->IsOpen()  && odFTmp->GetNkeys()){
	  cout << "TPC Chain: add file " << tpcFName << endl;
	  tpcChain.Add(tpcFName);
	  cout << "OD Chain: add file " << odFName << endl;
	  odChain.Add(odFName);
	  cout << "adding OD file to chain " << odFName << endl;
	  IsChained = true;
	}
	else{
	  cout << "cannot open file " << odFName << endl;
	}
      }
      else{
	cout << "cannot open file " << tpcFName << endl;
      }
