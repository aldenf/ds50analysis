...
void LoopOverChain(TChain* tpc_chain, TString outFileName)
{
  ...
  TH2F** max_s1_frac_total_s1_hists = new TH2F*[36];
  TH1F** max_s1_frac_hists = new TH1F*[37];
  TH1F*** max_s1_frac_hists_total_s1 = new TH1F**[37*15];
  ...
  for (int i=0; i<37; i++) {
    ...
    title<<"Max S1 / total S1 vs total S1, "<<i*10<<" < T_drift < "<<(i+1)*10<<"; total S1 [pe]; max S1 / total S1";
    max_s1_frac_total_s1_hists[i] = new TH2F(name.str().c_str(), title.str().c_str(), 500, 0, 500, 400, 0, 1);

    name.str(""); title.str("");
    name<<"max_s1_frac_hist_"<<i;
    title<<"Max S1 / total S1, 200<total_s1<500, "<<i*10<<" < T_drift < "<<(i+1)*10<<"; max S1 / total S1";
    ...
    for (int j=0; j<15; j++) {
      ...
      title<<"Max S1 / total S1, "<<j*10<<" < total S1 < "<<(j+1)*10<<", "<<i*10<<" < T_drift < "<<(i+1)*10<<"; max S1 / total S1";
  ...
  //for (int n=0; n<10000; n++)
    {
      ...
      Double_t max_s2_chan_radius = -1;
      if (max_s2_chan >= 19) 
        max_s2_chan_radius = TMath::Sqrt(pmt_xpos[max_s2_chan-19]*pmt_xpos[max_s2_chan-19]+
                                         pmt_ypos[max_s2_chan-19]*pmt_ypos[max_s2_chan-19]);
      ...
      */
      ...
      for (int i=0; i<37; i++) {
        if (t_drift > i*10. && t_drift <= (i+1)*10.) {
